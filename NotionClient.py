...
class NotionSyncPage:
    def __init__(self, connection_key, database_id ):
        self.notion       = Client(auth=connection_key)
        self.database_id  = database_id
        full_or_partial_pages = self.notion.pages.children(self.database_id)
        self.file_names   = {}
        for page in full_or_partial_pages["results"]:
            if page['type'] == 'paragraph':
                if len(page['paragraph']['rich_text']) != 0:
                    print(page['paragraph']['rich_text'][0]['plain_text'])
                continue
            elif page['type'] == 'child_page':
    ...
    def create_new_page(self, page_name, tags=[{"name": "general"}],**kargs):
        ...
        if page_name not in self.file_names:
            self.notion.pages.create(parent={"page_id": self.database_id}, properties=new_page)
            print(f"You add {page_name} page into page!")
        else:
            print(f"Use existed {page_name} page in page!")

    def get_page_id_via_name(self, page_name):
        results = self.notion.pages.children(self.database_id).get("results")
        for page in results:
            if page['type'] == 'child_page':
                if page['child_page']['title'] == page_name:
                    print(f"The first result is {page['child_page']['title']}")
                    return page['id']
